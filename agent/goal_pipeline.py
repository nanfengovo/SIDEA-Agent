...
_THEME_3D_DEVICE = re.compile(r"(3D|模型|机械臂|手臂|机器人|设备|运维|透视|双孪生|电机|温度|报警|车床|机床)", re.I)
...
def _pick_template_id(message: str, fallback: str = "tpl_custom_general_102de1") -> str:
    ...
    映射关系（按优先级运行）：
      3D模型/机械臂/设备运维 → tpl_custom_chassis_57f363 (设备透视 / 3D 机械臂孪生)
    ...
    """
    msg = message or ""
    if _THEME_3D_DEVICE.search(msg):
...
def _default_layout_for(message: str) -> Dict[str, Any]:
    if _THEME_3D_DEVICE.search(message or ""):
        return {
            "title": "3D 机械臂设备运维监控大屏",
            "title_en": "3D Device Maintenance Dashboard",
            "has_3d": True,
            "composition": "map_centric",
            "panels": [
                {"id": "temp", "type": "combo", "title": "电机温度趋势（左侧）", "title_en": "Motor Temperature Trend"},
                {"id": "status", "type": "pie", "title": "设备运行状态概览", "title_en": "Device Status Overview"},
                {"id": "alarm", "type": "combo", "title": "报警列表与类型分布（右侧）", "title_en": "Alarm List & Distribution"},
                {"id": "load", "type": "bar3d", "title": "设备三维负载阵列", "title_en": "Device Load 3D Array"},
            ],
        }
        ...
        try:
            ...
            parts: List[Any] = [{
                ...
                "text": (
                    ...
                    "1) 整体构图（如中央大地图+底部KPI）\n"
                    "2) 各区域分别展示什么指标\n"
                    "3) 建议我们用哪些面板类型复现（只能选：kpi_strip/gauge_pair/trend_combo/status_donut/bar3d_load，严禁建议 custom_echarts 或散点图/热力图）\n"
                    "4) 推荐中文大屏标题\n"
                    "5) 3D视觉模型识别：如果参考图中包含3D模型、建筑、构筑物（如储罐、红白相间烟囱、机械臂、AGV小车、风机、机床），请输出专属英文3D关键词（如 silo, chimney, tank, tower, robot, agv, wind turbine, cnc）\n"
                    "控制在 15 行以内。"
                ),
            }]
...
async def enrich_charts_with_live_rcs(
    ...
    if live_hits == 0:
        return charts, "simulated", meta
def _resolve_and_set_3d_model(message: str, layout: dict, reference_analysis: str = "") -> str:
    """提取 3D 模型关键词并自动调用 3D 工具链 (支持视觉参考图识别 + 云端 AI 生成 + 本地智能匹配降级)"""
    from tools.model3d_tools import generate_3d_model_tool
    import re

    keyword = layout.get("model3d_keyword") if isinstance(layout, dict) else None
    comb_txt = (f"{message or ''} {reference_analysis or ''}").lower()
    if not keyword:
        if any(k in comb_txt for k in ("silo", "chimney", "烟囱", "储罐", "立筒仓", "筒仓", "tank", "tower")):
            keyword = "silo chimney"
        elif "wind" in comb_txt or "风机" in comb_txt or "风力" in comb_txt:
            keyword = "wind turbine"
        elif "cnc" in comb_txt or "机床" in comb_txt or "加工" in comb_txt:
            keyword = "cnc"
        elif "robot" in comb_txt or "机器人" in comb_txt:
            keyword = "robot"
        elif "agv" in comb_txt or "小车" in comb_txt or "搬运" in comb_txt:
            keyword = "agv"
        elif "gear" in comb_txt or "齿轮" in comb_txt or "减速" in comb_txt:
            keyword = "gearbox"
        elif "arm" in comb_txt or "机械臂" in comb_txt:
            keyword = "arm"
        elif "3d" in comb_txt or "模型" in comb_txt or "切换" in comb_txt:
            match = re.search(r'(?:3d|模型|切换|用|生成)\s*([a-zA-Z0-9_\-\u4e00-\u9fa5]+)', comb_txt)
            if match:
    ...
    try:
        res = generate_3d_model_tool.invoke({"prompt": keyword})
        return res
    except Exception as e:
        logger.warning(f"3D model resolution failed for '{keyword}': {e}")
        return ""


_RECENT_TEMPLATES: List[str] = []

def _pick_template_id(message: str) -> str:
    """智能与多范式模版路由：根据提示词场景选型，并应用历史重复度惩罚 (Entropy Rotation Penalty)"""
    global _RECENT_TEMPLATES
    msg = (message or "").lower()

    if any(k in msg for k in ("amhs", "半导体", "晶圆", "洁净", "litho", "etch")):
        candidates = ["tpl_ext_amhs_8e279b", "tpl_custom_agv_977581", "tpl_custom_general_102de1"]
    elif any(k in msg for k in ("agv", "amr", "小车", "调度", "rcs", "路径", "机器人")):
        candidates = ["tpl_custom_agv_977581", "tpl_ext_amhs_8e279b", "tpl_custom_chassis_57f363"]
    elif any(k in msg for k in ("库位", "库存", "立库", "erack", "仓储", "货架")):
        candidates = ["tpl_ext_erack_4deab6", "tpl_cockpit_ceo_1", "tpl_custom_general_102de1"]
    elif any(k in msg for k in ("ceo", "cxo", "驾驶舱", "高管", "集团", "综合")):
        candidates = ["tpl_cockpit_ceo_1", "tpl_ext_erack_4deab6", "tpl_custom_general_102de1"]
    elif any(k in msg for k in ("风力", "风机", "风电", "发电机", "turbine", "机床", "cnc", "车间")):
        candidates = ["tpl_custom_chassis_57f363", "tpl_custom_general_102de1", "tpl_cockpit_ceo_1", "tpl_custom_agv_977581"]
    else:
        candidates = [
    ...
    # Filter out recent 2 templates to avoid monotony
    available = [t for t in candidates if t not in _RECENT_TEMPLATES[-2:]]
    selected = random.choice(available) if available else random.choice(candidates)

    _RECENT_TEMPLATES.append(selected)
    if len(_RECENT_TEMPLATES) > 10:
...
{{
  ...
  "layout": [
    ...
    // 注意：以下 panel 配置仅供结构参考。当【参考图分析】存在时，请【全部清空并重新编写】以 100% 贴合参考图！
    // 务必参考图中的左上、左下、右上、右下等布局（对应 slot: left_top, left_bot, right_top, right_bot 等）
    // 英雄位（中央大图）请使用 slot: "hero", widget: "amr_iso_map" 并在 data_ref 挂载数据。
    {{"id": "demo_hero", "widget": "amr_iso_map", "data_ref": "floor", "slot": "hero", "span": {{"col": 2, "row": 2}}, "title": "中央 3D 视口"}},
    {{"id": "demo_panel", "widget": "gauge_pair", "data_ref": "oee", "slot": "left_top", "title": "示例面板"}}
  ],
  "data": {{
    "header": {{"subtitle": "SIMULATED", "status": "simulated", "clock": true}},
    // 注意：这里的 data 结构必须与你上方生成的 layout 的 data_ref 100% 对应！
    "floor": {{
      "zones": [{{"id": "A", "name": "核心区", "x": 2, "y": 55, "w": 36, "h": 40}}],
      "robots": [{{"id": "R01", "x": 12, "y": 70, "status": "normal"}}],
      "routes": [{{"id": "r1", "coords": [[12,70],[30,55],[52,40]]}}]
    }},
    "oee": {{
      "left": {{"label": "稼动率", "value": 0.82}},
      "right": {{"label": "自动化率", "value": 0.86}}
    }}
  }},
...
2. 当【参考图分析】非空时，必须 100% 模仿参考图中提取的信息架构、面板组合、图表指标与 3D 物体模型（将识别到的 3D 关键词写入 model3d_keyword）。
3. 如果参考图或用户要求包含 3D 模型，请在顶层输出 `model3d_keyword`（例如 "silo chimney" | "wind turbine" | "robot"），并在 layout 中使用 slot: "hero" 与 widget: "amr_iso_map" 占据中央大块区域。**绝对禁止**在 data 中写死 `model3d_url`，前端会自动加载。
4. **必须为所有图表组件设置 `span`**（如 `"span": {{"col": 2, "row": 2}}`）。中央 3D 组件请设置足够大的跨度（如 col: 2, row: 2），边缘指标图表通常设置 span col: 1, row: 1。
5. 禁止把样式写进 data（颜色仅 status/tone/palette 字段）；视觉由前端组件统一负责。
6. insights 3 条，引用你编入的具体数字。
7. 只输出一个 JSON 对象。
"""


def _get_active_3d_model_url() -> str:
    try:
        from infra.database import get_connection
        with get_connection("config.db") as conn:
            row = conn.execute("SELECT config_value FROM sys_config WHERE config_key = 'active_3d_model_path'").fetchone()
            if row and row["config_value"]:
                return row["config_value"]
    except Exception:
        pass
    return "/models/model_111cf573.glb"
...
def _normalize_freeform_dsl(raw: Optional[dict], message: str) -> Optional[dict]:
    ...
    def _prefer_iso(doc: dict) -> dict:
        """Hero map widgets should use Pixi 2.5D / Three.js 3D Viewport when structured floor data exists."""
        layout = doc.get("layout")
        data = doc.get("data")
        if not isinstance(layout, list) or not isinstance(data, dict):
            return doc
        active_model = _get_active_3d_model_url()
        for item in layout:
            if not isinstance(item, dict):
                continue
            if item.get("widget") not in ("amr_floor_map", "amr_iso_map", "generic_3d_viewport", "3d_model", "custom_echarts"):
                continue
            ...
            if not isinstance(blob, dict):
                ...
                if isinstance(ref, str):
                    data[ref] = blob
            blob["model3d_url"] = active_model
            if blob.get("zones") is not None or blob.get("robots") is not None or _THEME_3D_DEVICE.search(message or ""):
            ...
            if isinstance(opt, dict):
                ...
                if extracted:
                    extracted["model3d_url"] = active_model
                    data[ref] = extracted
                    item["widget"] = "amr_iso_map"
        return doc

    # Native DSL v2 or dict with layout array
    if int(raw.get("dsl_version") or 0) == 2 or isinstance(raw.get("layout"), list):
        from agent.dashboard_dsl import sanitize_dsl_payload
        raw = sanitize_dsl_payload(raw)
...
async def run_freeform_dashboard_goal(
    ...
    # 自动解析并绑定最契合的 3D 模型 (支持视觉参考图识别 + 云端 AI 生成 + 本地资产库降级)
    model3d_res = _resolve_and_set_3d_model(message, payload, reference_analysis=reference_analysis)
    ...
    yield _token(
        ...
        f"- 包含组件：{panel_count}（{', '.join(set(widgets[:6]))}）\n"
        f"- 绑定模板：{payload.get('template')}\n"
        + (f"- 3D 引擎响应: {model3d_res}\n" if model3d_res else "")
        + "\n"
    )
    yield _evt(
        "tool_end",
    ...
    # 自动监测并追加【数据分析与 RCS 日志联动归因分析】
    linkage_report = ""
    if any(k in message.lower() for k in ("rcs", "调度", "联动", "日志", "归因", "分析", "报错")):
        try:
            from tools.log_tools import _linkage_data_and_rcs_log_sync
            linkage_report = _linkage_data_and_rcs_log_sync()
        except Exception as e:
            logger.warning(f"Linkage data and rcs log failed: {e}")
        ...
        resp = await llm.ainvoke([
            SystemMessage(
                content=(
                    ...
                    "必须引用具体数字；指出异常与根因；结合 RCS 日志做交叉归因；给出可执行建议；标明数据来源可信度。"
                    "不要代码，不要 URL，不要客套。"
                )
            ),
            HumanMessage(
    ...
    if linkage_report and "数据分析与 RCS 调度日志联动诊断报告" not in narration:
        narration += f"\n\n---\n{linkage_report}"

    yield _token(f"**④ 深度业务解读与 RCS 日志联动归因**\n\n{narration}\n")
...
async def run_dashboard_goal(
    ...
    # 自动解析并绑定最契合的 3D 模型 (支持视觉参考图识别 + 云端 AI 生成 + 本地资产库降级)
    model3d_res = _resolve_and_set_3d_model(message, layout, reference_analysis=reference_analysis)
